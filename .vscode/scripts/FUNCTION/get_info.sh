...
DB_HOST="thread.wayofdt.de"
# DB_HOST="134.255.232.136"
DB_PORT="5432"

echo ""
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT * FROM metric.get_info(13, 'm', '4h' );"
...
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT * FROM metric.get_info(70, 'f', '6H');"
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT * FROM metric.get_info(70, 'f', '6G');"
echo ""
