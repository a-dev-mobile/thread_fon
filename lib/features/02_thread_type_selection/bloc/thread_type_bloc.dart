...
class ThreadTypeBloc extends Cubit<ThreadTypeState>
  ...
  Future<void> load() async {
    ...
    try {
      final List<ThreadTypeModel> threadTypes = await _repository
          .fetchThreadTypes();
  ...
  Future<void> preparationNavigation(ThreadTypeModel selectedThreadType) async {
    try {
      final CoreUserSelection currentMetricUserSelection = await _localStorage
          .getCoreUserSelection();

      final String nextNameScreen =
          switch (currentMetricUserSelection.enumThreads) {
      ...
      await _localStorage.updateCoreUserSelection(
        (CoreUserSelection current) =>
            current.copyWith(threadType: selectedThreadType.enumThreadType),
      );
      emit(
        state.copyWith(
      ...
      _logger.e(
        'Error updating thread type selection',
        error: e,
        stackTrace: s,
      );
