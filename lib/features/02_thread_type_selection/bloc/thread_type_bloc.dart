...
class ThreadTypeBloc extends Cubit<ThreadTypeState> with BlocIgnoreEmitAfterClosed {
  ...
  Future<void> load() async {
    ...
    try {
      final List<ThreadTypeModel> threadTypes = await _repository.fetchThreadTypes();
      final CoreUserSelection coreUserSelection = await _localStorage.getCoreUserSelection();
      emit(state.copyWith(
  ...
  Future<void> preparationNavigation(ThreadTypeModel selectedThreadType) async {
    try {
      final CoreUserSelection currentMetricUserSelection = await _localStorage.getCoreUserSelection();

      final String nextNameScreen = switch (currentMetricUserSelection.enumThreads) {
